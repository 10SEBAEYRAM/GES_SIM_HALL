    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-gradient-to-br from-blue-50 via-white to-gray-50 overflow-hidden shadow-lg sm:rounded-lg p-6 border border-gray-300">
            <div class="flex justify-between items-center mb-6 border-b border-gray-300 pb-4">
                <h2 class="text-3xl font-bold text-gray-700">Transactions</h2>
            ...
            <div class="mb-4 flex gap-4 animate-fade-in-down">
                ...
                <select id="filterCaisse" class="border-gray-300 rounded-lg p-3 shadow-md">
                    <option value="">Toutes les Caisses</option>
                    @foreach($caisses as $caisse)
                    <option value="{{ $caisse->nom_caisse }}">{{ $caisse->nom_caisse }}</option>
                    @endforeach
                </select>
            ...
            <!-- Tableau -->
            <table id="transactionsTable" class="min-w-full divide-y divide-gray-200 bg-white shadow-lg rounded-lg">
                ...
                <tbody>
                    ...
                    <tr class="hover:bg-gray-100 transition duration-200">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">{{ $transaction->caisse->nom_caisse }}</td>
                        <td class="px-6 py-4 text-right text-sm font-medium">
                            <div class="flex gap-3 justify-end">
                                <a href="{{ route('transactions.show', $transaction->id_trans) }}"
                                ...
                                <form action="{{ route('transactions.destroy', $transaction->id_trans) }}" method="POST" class="inline-block">
                                    @csrf
                                    @method('DELETE')
                                    <button
                                        type="submit"
                                        ...
                                        onclick="return confirm('Êtes-vous sûr de vouloir annuler cette transaction ? Cette action inversera tous les mouvements de compte.')">
                                        Annuler
                                    </button>
                                </form>
                                @endif
...
<script>
    $(document).ready(function() {
        ...
        $('#filterCaisse').on('change', function() {
        ...
        $('#filterMinMontant').on('input', function() {
            $.fn.dataTable.ext.search.push(
                function(settings, data) {
                    var min = parseFloat($('#filterMinMontant').val()) || 0;
                    var balance = parseFloat(data[3].replace(/\s/g, '').replace('FCFA', '')) || 0;
                    return balance >= min;
                }
            );
            table.draw();
        ...
        $('#filterMaxMontant').on('input', function() {
            $.fn.dataTable.ext.search.push(
                function(settings, data) {
                    var max = parseFloat($('#filterMaxMontant').val()) || Infinity;
                    var balance = parseFloat(data[3].replace(/\s/g, '').replace('FCFA', '')) || 0;
                    return balance <= max;
                }
            );
